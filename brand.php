...
<div class="container">
    ...
    <div class="row">
        ...
        <div class="col-lg-9">
            <?php
            $brand_id = $_GET['brand'];
            $get_pro = "SELECT * from brands INNER JOIN products ON products.product_brand = brands.brand_id WHERE brand_id = '$brand_id' ";
            $run_pro = mysqli_query($con, $get_pro);
            $get_brand_title = "SELECT * FROM brands WHERE brand_id = '$brand_id'";
            $run_brand_title = mysqli_query($con, $get_brand_title);
            while ($row_title = mysqli_fetch_array($run_brand_title)) {
                $brand_title = $row_title['brand_title'];
            }
            ?>
            <h1 style="margin-top:40px; padding-bottom:10px; text-align:center"><?= $brand_title ?></h1>
            <div class="row">
                <?php
